...
class PredictionOut:
    ...
    Attributes:
        ...
        prediction (PredictionOutPrediction): Prediction result. Since 1.0.0 'value' is the LEVEL by default: for a
            ...
            prediction's two-axis confidence >= tau, 'climatology_floor' when below tau — the value is then the fit-window
            level mean, a genuine non-persistence anchor). 'confidence' is the per-point confidence in [0,1].
            'confidence_basis' is the structured certificate (method, in_range, sigma_clim, interval_half_width,
            uncertified_reason). Null for legacy models without fit-time artifacts.
