...
class PredictionOutPrediction:
    ...
    of the point ('neural_scored' when the GBT prediction's two-axis confidence >= tau, 'neural_weak' when below tau —
    the raw GBT prediction is always served with its confidence metric, never replaced; 'no_forecast' when no model
    exists — value is null). 'confidence' is the per-point confidence in [0,1]. 'confidence_basis' is the structured
    certificate (method, in_range, sigma_clim, interval_half_width, uncertified_reason). Null for legacy models without
    fit-time artifacts.
